...
/* colour cycling nautilus note 3*/
static const unsigned int color_list[] = {
  LCD_RGBPACK(255,   0,   0),   /* red */
  LCD_RGBPACK(255, 120,   0),   /* orange */
  LCD_RGBPACK(255, 191,   0),   /* amber */
  LCD_RGBPACK(255, 255,   0),   /* yellow */
  LCD_RGBPACK(127, 255,   0),   /* chartreuse */
  LCD_RGBPACK(  0, 255,   0),   /* lime */
  LCD_RGBPACK(  0, 255, 127),   /* spring green */
  LCD_RGBPACK(  0, 206, 209),   /* dark turquoise */
  LCD_RGBPACK(  0, 255, 255),   /* cyan */
  LCD_RGBPACK(  0, 191, 255),   /* deep sky blue */
  LCD_RGBPACK(  0,   0, 255),   /* blue */
  LCD_RGBPACK(138,  43, 226),   /* blue violet */
  LCD_RGBPACK(148,   0, 211),   /* violet */
  LCD_RGBPACK(255,   0, 255),   /* magenta */
  LCD_RGBPACK(255, 255, 255),   /* white */
};
#define COLOR_COUNT 15
static int color_index = 0;     /* peaks view cursor — unchanged */

/* waveform: independent per-channel trackers */
/* color_index_L/R are now computed per-frame, not stored nautilus note 17 */
static int audio_energy_L = 0;
static int audio_energy_R = 0;
static int peak_energy_L  = 1;
static int peak_energy_R  = 1;
static int frame_peak_amp_L = 0;
static int frame_peak_amp_R = 0;
...
static void anim_draw_cursor_h(int x)
{
    ...
    rb->lcd_set_foreground(color_list[color_index]); color_index = (color_index + 1) % COLOR_COUNT;;
#else
...
static void anim_waveform_plot_pixel_h(int x, int x_prev,
                                         int vmin, int vmax,
                                         int vmin_prev, int vmax_prev)  /*nautilus note 6*/
{
...
static int get_energy_color(int peak_amp, int *energy, int *peak)
{
    /* attack: jump up instantly, decay slowly nautilus note 18 */
    if (peak_amp > *energy)
        *energy = peak_amp;
    else
        *energy = (*energy * 9) / 10;
    ...
    int level_pct = (*energy * 100) / *peak;

    /* high energy → low index (red), low energy → high index (blue/white) */
    return ((100 - level_pct) * (COLOR_COUNT - 1)) / 100;
}
...
static long anim_waveform_horizontal(void)
{
    ...
    rb->lcd_set_drawmode(DRMODE_SOLID);
#if LCD_DEPTH > 1    /*nautilus note 8*/
    int color_index_L = get_energy_color(frame_peak_amp_L, &audio_energy_L, &peak_energy_L);
    int color_index_R = get_energy_color(frame_peak_amp_R, &audio_energy_R, &peak_energy_R);
    frame_peak_amp_L = 0;
    frame_peak_amp_R = 0;
    /* foreground is set per-channel inside the drawing loop below nautilus note 8 */
#endif
    ...
    int lmin, lmin_prev = LCD_HEIGHT / 4;
    int rmin, rmin_prev = (3 * LCD_HEIGHT) / 4;
    int lmax, lmax_prev = LCD_HEIGHT / 4;
    int rmax, rmax_prev = (3 * LCD_HEIGHT) / 4;
    ...
    /* track raw amplitude before screen conversion naut note 9 this might part might be broken */
        if (x_prev >= 0)
        {
            int lamp = lmax - lmin;
            int ramp = rmax - rmin;
            if (lamp > frame_peak_amp_L) frame_peak_amp_L = lamp;
            if (ramp > frame_peak_amp_R) frame_peak_amp_R = ramp;
        }
...
#if LCD_DEPTH > 1
        rb->lcd_set_foreground(color_list[color_index_L]);
#endif
        plot[osc.draw](x, x_prev, lmin, lmax, lmin_prev, lmax_prev);
#if LCD_DEPTH > 1
        rb->lcd_set_foreground(color_list[color_index_R]);
#endif
    ...
    plot_start_noprev:
       ...
       /* nautilus note 11 this part might break everything too */
        if (x_prev >= 0)
        {
            int lamp = lmax - lmin;
            int ramp = rmax - rmin;
...
static void anim_waveform_plot_pixel_v(int y, int y_prev,  /*nautilus note 6 ot 7 idk*/
                                         int vmin, int vmax,   
                                         int vmin_prev, int vmax_prev)
{
...
static long anim_waveform_vertical(void)
{
    ...
    rb->lcd_fillrect(0, 0, LCD_WIDTH, LCD_HEIGHT);
    rb->lcd_set_drawmode(DRMODE_SOLID);
#if LCD_DEPTH > 1  /*nautilus note 13 this is for the vertical graph*/
    int color_index_L = get_energy_color(frame_peak_amp_L, &audio_energy_L, &peak_energy_L);
    int color_index_R = get_energy_color(frame_peak_amp_R, &audio_energy_R, &peak_energy_R);
    frame_peak_amp_L = 0;
    frame_peak_amp_R = 0;
    ...
    /* track raw amplitude before screen conversion nautilus note 14 this might break*/ 
    ...
    plot_start_noprev:
        ...
        /* track raw amplitude before screen conversion nautilus note 16 this is dodgy code */
        {
            int lamp = lmax - lmin;
            int ramp = rmax - rmin;
            if (lamp > frame_peak_amp_L) frame_peak_amp_L = lamp;
