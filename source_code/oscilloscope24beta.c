...
static const unsigned int color_list[] = {
    ...
    LCD_RGBPACK(255, 215, 0),    /* gold */ 
    LCD_RGBPACK(255, 140, 0),    /* dark orange */ 
    ...
    LCD_RGBPACK(255,   255,   255),  /* white */  
    LCD_RGBPACK(255, 20,  147),  /* deep pink */
};
#define COLOR_COUNT 12
static int color_index      = 0;
static int audio_energy     = 0;   /*after colour_index this is the claude ai stuff*/
static int peak_energy      = 1;   /* starts at 1 to prevent divide-by-zero */
static int color_tick_accum = 0;
static int frame_peak_amp   = 0;   /* filled by plot functions, read each frame */

/* Tuning — adjust these to taste */
#define SPEED_MIN  4    /* ticks per colour step at loudest (faster) */
#define SPEED_MAX  30   /* ticks per colour step at silence (slower)  */
...
static void anim_waveform_plot_pixel_h(int x, int x_prev,    /*nautilus note 6 claude added something to horizontal*/
                                       int vmin, int vmax,
                                       int vmin_prev, int vmax_prev)
{
    int amp = vmax - vmin;                      /* height of waveform at this column */
    if (amp > frame_peak_amp) frame_peak_amp = amp;  /* keep running max for this frame */
...
static void update_beat_color(int peak_amp) /*nautilus_note 4 sketchy claude function*/
{
    /* Attack: jump to louder value instantly (catches beats immediately) */
    if (peak_amp > audio_energy)
        audio_energy = peak_amp;
    else
        audio_energy = (audio_energy * 9) / 10;  /* decay: *0.9, no floats */

    /* Track loudest moment seen for self-calibration */
    if (audio_energy > peak_energy)
        peak_energy = audio_energy;

    /* level_pct: 0 = silent, 100 = loudest moment ever heard */
    int level_pct = (audio_energy * 100) / peak_energy;

    /* High energy → small ticks_per_step → colour advances faster */
    int ticks_per_step = SPEED_MAX - ((level_pct * (SPEED_MAX - SPEED_MIN)) / 100);

    color_tick_accum++;
    if (color_tick_accum >= ticks_per_step)
    {
        color_tick_accum = 0;
...
static long anim_waveform_horizontal(void)
{
    ...
    rb->lcd_set_drawmode(DRMODE_SOLID);
#if LCD_DEPTH > 1
    update_beat_color(frame_peak_amp);  /* use last frame's peak, update color_index */
    frame_peak_amp = 0;                 /* reset so this frame's plots fill it fresh */
    rb->lcd_set_foreground(color_list[color_index]); /*nautilus_note_1_horizontal*/
#endif
...
static void anim_waveform_plot_pixel_v(int y, int y_prev,
                                       ...
                                       int vmin_prev, int vmax_prev)
{
    int amp = vmax - vmin;         /*nautilus note 7 more claude stuff*/
    if (amp > frame_peak_amp) frame_peak_amp = amp;
...
static long anim_waveform_vertical(void)
{
    ...
    rb->lcd_set_drawmode(DRMODE_SOLID);
#if LCD_DEPTH > 1
    update_beat_color(frame_peak_amp);  /* same shared function and state */
    frame_peak_amp = 0;
    rb->lcd_set_foreground(color_list[color_index]); /*nautilus_note_2_vertical*/
#endif
